...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE parent_object_id = OBJECT_ID('dbo.ESTOQUE_PRODUTO')
          AND name = 'DF_ESTOQUE_PRODUTO_USUARIO'
    )
        ALTER TABLE [dbo].[ESTOQUE_PRODUTO]
          ADD CONSTRAINT DF_ESTOQUE_PRODUTO_USUARIO DEFAULT('HRM') FOR [USUARIO];
...
BEGIN
    ...
    DECLARE @maxFornecedorCodigo int = (SELECT ISNULL(MAX(CODIGO), 0) FROM [dbo].[FORNECEDOR]);
